...
def extract_resnet_features(images, batch_size=32):
    """Extract features using pre-trained ResNet50 with partial fine-tuning"""
    ...
    # ResNet50 has different layer structure than VGG16,
    # so we need to adjust the freezing strategy
    
    # Freeze first 4 blocks (layers up to approximately 100)
    for layer in base_model.layers[:100]:
        layer.trainable = False
    # Make the rest trainable
    for layer in base_model.layers[100:]:
...
def create_improved_model(input_shape, num_classes):
    ...
    # ResNet features are higher dimensional, so we adjust the architecture
    # First block
    ...
    # Third block with residual connection - adjusted for dimensionality
    ...
    optimizer = Adam(learning_rate=0.0003)  # Slightly lower learning rate for ResNet
    model.compile(
        optimizer=optimizer,
...
def main():
    ...
    # 5. Extract features using ResNet50 instead of VGG16
    print("Extracting features using pre-trained ResNet50...")
    train_features = extract_resnet_features(train_images)
    val_features = extract_resnet_features(val_images)
    ...
    # Define callbacks - increased patience for ResNet which may converge differently
    callbacks = [
    ...
    history = model.fit(
        ...
        epochs=60,  # Slightly more epochs for ResNet
        batch_size=24,  # Smaller batch size due to ResNet's higher memory requirements
        callbacks=callbacks,
    ...
    # 11. Save the model
    model.save('resnet50_xray_classifier.h5')
